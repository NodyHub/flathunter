...
class WGSearcher:
	...
	def extract_data(self, soup):
		...
		existingFindings = list(filter(lambda e : e.has_attr('class') and not 'listenansicht-inactive' in e['class'], findings))
		...
		for row in existingFindings:
			...
			price = row.find(lambda e : e.has_attr('class') and 'ang_spalte_miete' in e['class']).text.strip() # u'433\u20ac'
			size = row.find(lambda e : e.has_attr('class') and 'ang_spalte_groesse' in e['class']).text.strip() # u'75m\xb2'
			district = row.find(lambda e : e.has_attr('class') and 'ang_spalte_stadt' in e['class']).text.strip() # u'Altstadt-Lehel'
			date = row.find(lambda e : e.has_attr('class') and 'ang_spalte_freiab' in e['class']).text.strip() # u'21.03.17'
